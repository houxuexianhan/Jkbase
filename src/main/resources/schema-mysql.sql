...
--
 ...
 DROP VIEW IF EXISTS `view_sys_role_app`;
/*!50001 SET @saved_cs_client          = @@character_set_client */;
/*!50001 SET @saved_cs_results         = @@character_set_results */;
/*!50001 SET @saved_col_connection     = @@collation_connection */;
/*!50001 SET character_set_client      = utf8mb4 */;
...
--
 ...
 Create VIEW `view_sys_user_role_module` AS select `r`.`R_UserID` AS `R_UserID`,`r`.`R_RoleID` AS `R_RoleID`,`p`.`P_RoleID` AS `P_RoleID`,`p`.`P_ModuleID` AS `P_ModuleID`,`p`.`P_Value` AS `P_Value`,`m`.`ModuleID` AS `ModuleID`,`m`.`M_AppID` AS `M_AppID`,`m`.`M_CName` AS `M_CName`,`m`.`M_Code` AS `M_Code`,`m`.`M_Icon` AS `M_Icon`,`m`.`M_IsClose` AS `M_IsClose`,`m`.`M_OrderLevel` AS `M_OrderLevel`,`m`.`M_ParentID` AS `M_ParentID`,`m`.`M_URL` AS `M_URL`,`m`.`A_AppName` AS `A_AppName`,`m`.`M_ParentName` AS `M_ParentName` from ((`sys_user_role` `r` join `view_sys_user_role` `p` on(((`r`.`R_RoleID` = `p`.`P_RoleID`) and (`p`.`P_Value` > 0)))) join `view_sys_module` `m` on((`m`.`ModuleID` = `p`.`P_ModuleID`))) order by `m`.`M_AppID`,`m`.`M_OrderLevel` ;
/*
drop view if exists View_sys_Module;
...
drop view if exists View_sys_Function;
CREATE VIEW  View_sys_Function AS SELECT a.*,b.*,c.*,(select M_CName from sys_Module x where x.ModuleID=b.M_ParentID  )as M_ParentName FROM sys_Function AS a INNER JOIN sys_Module AS b ON a.F_ModuleId = b.ModuleID inner join sys_App AS c ON b.M_AppID = c.AppID;
drop view if exists View_sys_Company;
...
drop view if exists view_sys_user_role;
CREATE VIEW `view_sys_user_role` AS select DISTINCT *  from(select * from sys_user_role union all select userid,cr.roleid from sys_user u inner join sys_company_role cr on u.u_companyid=cr.companyid)v;
